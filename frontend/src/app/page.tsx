    ...
    <div className="min-h-screen bg-background text-foreground bg-grid">
        ...
        <div className="mx-auto max-w-[1240px] px-6 h-14 flex items-center">
          ...
          <div className="ml-auto flex items-center gap-2 text-[13px]">
            <nav className="flex items-center gap-4">
              <ThemeToggle />
              {user ? (
                <Link href="/dashboard" className="px-3 py-1.5 text-foreground/70 hover:text-foreground transition-colors">Dashboard</Link>
              ) : (
                <>
                  <Link href="/login" className="px-3 py-1.5 text-foreground/70 hover:text-foreground transition-colors">Log in</Link>
      ...
      <section className="mx-auto max-w-[1240px] px-6 pt-16 pb-24 grid lg:grid-cols-[1fr_1.05fr] gap-14 items-center">
        <div>
          ...
          <h1 className="font-display text-[52px] leading-[1.02] tracking-[-0.02em] max-w-[560px] text-foreground">
            Practice OAs that actually feel like the real thing.
          </h1>
          <p className="mt-6 text-[16px] text-foreground/70 max-w-[520px] leading-relaxed">
          ...
          <div className="mt-8 flex items-center gap-3">
            ...
            <Link href="/library" className="px-4 py-2.5 rounded-sm border border-border text-[14px] hover:bg-foreground/5 transition-colors font-display">
              Browse the library
            </Link>
          </div>
          <div className="mt-10 flex gap-8 stamp-id">
            <div><span className="text-foreground text-[13px] font-mono">MSFT · OA</span><div>MICROSOFT</div></div>
            <div><span className="text-foreground text-[13px] font-mono">AMZN · SDE1</span><div>AMAZON</div></div>
            <div><span className="text-foreground text-[13px] font-mono">GOOG · L3</span><div>GOOGLE</div></div>
          </div>
        </div>

        {/* Demo card */}
        <div className="stamp-card p-6 bg-background">
          ...
          <div className="border border-border p-4 mb-3">
            ...
            <div className="font-mono text-[13px] text-foreground">Microsoft OA · Graphs</div>
          </div>
          <div className="flex items-center gap-2 mb-3 text-[11px] font-mono text-foreground/60">
          ...
          <div className="border border-border p-4">
            ...
            <p className="text-[13px] text-foreground/70 leading-relaxed">
              You are given <span className="font-mono text-foreground">n</span> nodes and a log of directed broadcasts. Reconstruct the smallest connected component set such that every broadcast is causally consistent…
            </p>
            <div className="mt-3 flex gap-2 stamp-id">
              <span className="border border-border px-1.5 py-0.5">GRAPHS</span>
              <span className="border border-border px-1.5 py-0.5">UNION-FIND</span>
              <span className="border border-border px-1.5 py-0.5">TOPO-SORT</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1240px] px-6 py-20 grid md:grid-cols-3 gap-10">
          {FEATURES.map((f) => (
            <div key={f.id}>
              ...
              <p className="text-[14px] text-foreground/70 leading-relaxed">{f.b}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1240px] px-6 py-24">
        <div className="grid lg:grid-cols-[1fr_1.2fr] gap-12 items-center">
          <div>
            ...
            <p className="text-[15px] text-foreground/70 leading-relaxed max-w-[440px]">
